...
public class RecycleFilmeAdapter extends RecyclerView.Adapter<RecycleFilmeAdapter.ViewHolder> {
    ...
    public RecycleFilmeAdapter(Context context){
    ...
    public ViewHolder onCreateViewHolder(ViewGroup viewGroup, int viewType) {
        if(viewType==2){
            View v = inflater.inflate(R.layout.item_loader, viewGroup, false);
            RecycleFilmeAdapter.ViewHolder mvh = new RecycleFilmeAdapter.ViewHolder(v);
            return mvh;
        }else{
            View v = inflater.inflate(R.layout.item_produto, viewGroup, false);
            RecycleFilmeAdapter.ViewHolder mvh = new RecycleFilmeAdapter.ViewHolder(v);
            return mvh;
        }
    ...
    private  void prepare(final ViewHolder viewHolder, final Filme selector, final int position){
        viewHolder.nome.setText(selector.getOriginalTitle());
        viewHolder.categoria.setText(selector.getTitle());


    try {
        Picasso.with(context).load(selector.getPosterPath())
                .fit().centerCrop()
